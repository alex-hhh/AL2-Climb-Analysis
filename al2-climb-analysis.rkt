...
;; NOTE: this application uses racket packages that need to be installed using
;; a "raco pkg installed" command, see comments below for the list of
;; packages.
...
(define (get-pref key default)
  (get-preference key (lambda () default)))
      ...
      (define renderers
        (if (and alt grade)
            (let* ([c (find-climb the-climbs dst)]
                   [climb-badge (and c
                                     (make-hover-badge
                                      `(("Score"  ,(~r (climb-score c) #:precision 0))
...
(define (save-preferences)
  ...
  (unless (or (send toplevel is-maximized?) (send toplevel is-fullscreened?))
    (let-values (([w h] (send toplevel get-size)))
      (put-preferences '(al2-climb-analysis-tool:frame-dimensions) (list (cons w h)))))
  (put-preferences
